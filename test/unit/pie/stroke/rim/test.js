...
const rim = require('pie/stroke/rim');
//const wr = require('dev-utils/write-png-file-from-base-64');

const pi = Math.PI;

tp('rim down, one slice, it does not change the data', t => {
  const canvWidth = 400;
...
tp('rim down, 2 slices, it does not change the data', t => {
  const canvWidth = 400;
  const canvHeight = 300;
  const canvas = createCanvas(canvWidth, canvHeight);
  const cntx = canvas.getContext('2d');
...
tp('rim down, 3 slices, it does not change the data', t => {
  const canvWidth = 400;
  const canvHeight = 300;
  const canvas = createCanvas(canvWidth, canvHeight);
  const cntx = canvas.getContext('2d');
...
tp('rim up, 1 slice, it does not change the data', t => {
  const canvWidth = 400;
  const canvHeight = 300;
  const canvas = createCanvas(canvWidth, canvHeight);
  const cntx = canvas.getContext('2d');
...
tp('rim up, 2 slices, it does not change the data', t => {
  const canvWidth = 400;
  const canvHeight = 300;
  const canvas = createCanvas(canvWidth, canvHeight);
  const cntx = canvas.getContext('2d');
...
tp('rim up, 3 slices, it does not change the data', t => {
  const canvWidth = 400;
  const canvHeight = 300;
  const canvas = createCanvas(canvWidth, canvHeight);
  const cntx = canvas.getContext('2d');
